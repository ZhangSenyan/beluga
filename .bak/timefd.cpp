...
int timerfd_init(int ms)
{
    ...
    if (tmfd < 0) {
        std::cout<<"timerfd_create error, Error:["<<errno<<":"<<strerror(errno)<<"]"<<std::endl;
        return -1;
    }
    ret = timerfd_settime(tmfd, 0, &new_value, NULL);
    if (ret < 0) {
        std::cout<<"timerfd_settime error, Error:["<<errno<<":"<<strerror(errno)<<"]"<<std::endl;
        close(tmfd);
        return -1;
    }
